...
<body>
    ...
    <div class="left-box">
        <div class="menu">
            ...
            <div class="menu-content">
                ...
                <li>
                    <a href="<c:url value="/CaseAction?operate=listCase"></c:url>" class="active"><i class="fa fa-folder-open">&nbsp;&nbsp;共享案例</i></a>
                </li>
                </c:if>
    ...
    <div class="right-box container-fluid top-h">
        <div class="search">
            <form class="bs-example bs-example-form" role="form" action="<c:url value="/CaseAction"></c:url>" method="get">
                <div class="row">
                    <div class="col-xs-5 pull-right">
                        <div class="input-group">
                        <input type="hidden" name="operate" value="listCase">
        ...
        <div class="list">
            ...
            <div class="table-list" >
                ...
                <c:forEach items="${caseList}" var="caseInfo" varStatus="idx" >
                    <li>
                    	<c:if test="${sessionScope.userInfo.id==caseInfo.user.id}">
                        <a href="<c:url value='/HomeworkAction?operate=listHomework&caseId=${caseInfo.id}'></c:url>">
                    </a>
                    </c:if>
                        <table>
                            <tr>
                                <td  width="5%">${idx.index+1}</td>
                                <td  width="45%" class="quantitys">${caseInfo.caseName }</td>
                                <td  width="10%">${caseInfo.user.userName }</td>
                                <td  width="15%">${caseInfo.createDate }</td>
                                <td  width="5%">
                                    <a href="#" class="tooltip-test annex" data-toggle="tooltip" title="查看附件" onclick="showAppendix('${caseInfo.id}') ">
                                        <i class="fa fa-file-code-o"></i>
                                    </a>
                                </td>
                                <td  width="20%">
                                <c:if test="${sessionScope.userInfo.id==caseInfo.user.id}">
                                <c:if test="${fn:contains(sessionScope.listUrl,'updateCase')}">
                                <button type="button" class="btn btn-primary btn-xs details" onclick="updateCase('${caseInfo.id}')">
                                    <i class="fa fa-edit">&nbsp;编辑</i>
                                </button>
                                </c:if>
                                </c:if>
                                <c:if test="${fn:contains(sessionScope.listUrl,'addHomework')}">
                                <button type="button" class="btn btn-primary btn-xs upload" onclick="addHomework('${caseInfo.id}')">
                                    <i class="fa fa-upload">&nbsp;上传作业</i>
                                </button>
                                </c:if>
                                <c:if test="${sessionScope.userInfo.id==caseInfo.user.id}">
                                 <c:if test="${fn:contains(sessionScope.listUrl,'deleteCase')}">
                                <button type="button" class="btn btn-danger btn-xs" onclick="deleteCase('${caseInfo.id}')">
	...
	<c:if test="${pageNo > 1 }">
		<li><a href="<c:url value='/CaseAction?operate=listCase&search=${search}&pageNo=${pageNo-1 }'/>">&laquo;</a></li>
	</c:if>
	...
	<c:forEach begin="${begin }" end="${end }" var="i">
	  <c:choose>
	  	...
	  	<c:otherwise>
	  		<li><a href="<c:url value='/CaseAction?operate=listCase&search=${search}&pageNo=${i}'/>">${i }</a></li>
	  	</c:otherwise>
	  </c:choose>
	</c:forEach>
	...
	<c:if test="${pageNo < totalPage }">　
		<li><a href="<c:url value='/CaseAction?operate=listCase&search=${search}&pageNo=${pageNo+1 }'/>">&raquo;</a></li>
	</c:if>
</ul>　
        </div>
    </div>
    ...
    <script>
    function showAppendix(id) {
    	  layer.open({
              type: 2,
              skin: 'layui-layer-rim', //加上边框
              title:'',
              area: ['800px', '500px'], //宽高
              content: "<c:url value='/CaseAction?operate=showAppendix&caseId="+id+"'></c:url>"
          });
	}
    function updateCase(id) {
    	layer.open({
            ...
            title:'',
            area: ['900px', '570px'], //宽高
            content: "<c:url value='/CaseAction?operate=updateCase&caseId="+id+"'></c:url>"
        });
    }
    function addHomework(id) {
    	layer.open({
            ...
            skin: 'layui-layer-rim', //加上边框
            area: ['750px', '450px'], //宽高
            content: "<c:url value='/HomeworkAction?operate=addHomework&caseId="+id+"'></c:url>"
        });
    }
    function deleteCase(id) {
    	$.ajax({
    		...
    		type:'post',
    		dataType:'json',
    		data:{operate:"deleteCase",caseId:id},
