			...
			<div class="row">
				<div class="col-xs-6">
					<form class="form-search"
						action="<c:url value="/MobilePhoneInfoController?operate=list"/>"
						method="post">
						<div class="input-append">
							<div class="col-sm-5">
								<input type="hidden" name="pageNo" value="${pageNo}"> <input
									type="hidden" name="totalPage" value="${totalPage}"> <input
									type="hidden" name="brandName" value="${brandName}"> <input
							...
							<div class="products">
								...
								<div class="container">
									<form
										action="<c:url value="/MobilePhoneInfoController?operate=list"/>"
										method="post">
										品牌:<input type="text" name="brandName" value="${brandName}">
										型号:<input type="text" name="model" value="${model}"> <select
											class="btn btn-default" name="state" title="是否上架">
											<option value="">是否上架</option>
											<option value="0">未上架</option>
											<option value="1">已上架</option>
										</select> <input type="submit" class="btn btn-info" value="查询"
											onclick="changePage('frist')"> <a
											href="<c:url value="/MobilePhoneInfoController?operate=list"/>"
									...
									<table class="table table-hover table-bordered ">
										<thead>
											<tr>
												...
												<th width="50" align="left">图片</th>
												<th width="120" align="left">品牌</th>
												<th width="120" align="left">型号</th>
												<th width="120" align="left">网络类型</th>
												<th width="120" align="left">屏幕尺寸</th>
												<th width="100" align="left">价格</th>
												<th width="100" align="left">售价</th>
												<th colspan="3"><div class="text-center">操作</div></th>
											</tr>
										</thead>
										<tbody>
											<c:forEach items="${mobilePhoneInfoList}"
												var="mobilePhoneInfo" varStatus="index">
												<tr>
													<td>${index.count}</td>
												...
												images/default.jpg
													...
													<td><a class="btn btn-2 "
														href="<c:url value="/MobilePhoneInfoController?operate=changeState&mobPhoneId=${mobilePhoneInfo.mobPhoneId}&state=${mobilePhoneInfo.state}"></c:url>">
															<c:if test="${mobilePhoneInfo.state==0}">上架</c:if> <c:if
																test="${mobilePhoneInfo.state==1}">下架</c:if>
													</a> <a class="btn btn-2 "
														href=" <c:url value="/MobilePhoneInfoController?operate=show&mobPhoneId=${mobilePhoneInfo.mobPhoneId}"></c:url>">查看</a>
														<a class="btn btn-2 "
														href="<c:url value="/MobilePhoneInfoController?operate=edit&mobPhoneId=${mobilePhoneInfo.mobPhoneId}"></c:url>">编辑
													</a> <a class="btn btn-2 "
														href="<c:url value="/MobilePhoneInfoController?operate=delete&mobPhoneId=${mobilePhoneInfo.mobPhoneId}"></c:url>"><i
