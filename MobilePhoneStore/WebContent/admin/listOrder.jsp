...
<body>
			...
			<div class="row">
				<div class="col-xs-6">
					<form class="form-search"
						action="<c:url value="/OrderController?operate=list"/>"
						method="post">
						<div class="input-append">
							<div class="col-sm-5">
								<input type="hidden" name="pageNo" value="${pageNo}"> <input
									...
									type="hidden" name="isPay" value="${isPay}"> <input
									type="hidden" name="orderId" value="${orderId}"> <input
									type="hidden" name="isDeliver" value="${isDeliver}"> <input
	...
	<div id="page-content" class="single-page">
		<div class="container">
			<div class="container">
				<form action="<c:url value="/OrderController?operate=list"/>"
					method="post">
					订单号:<input type="text" name="orderId" value="${orderId}">&nbsp&nbsp
					用户名: <input type="text" name="loginName" value="${loginName}">&nbsp&nbsp
					<select class="btn btn-default" name="isPay" title="是否付款">
						<option value="">是否付款</option>
						<option value="0">未付款</option>
						<option value="1">已付款</option>
					</select>&nbsp&nbsp <select class="btn btn-default" name="isDeliver"
						title="是否发货">
						<option value="">是否发货</option>
						<option value="0">未发货</option>
						<option value="1">已发货</option>
					</select> &nbsp&nbsp<input type="submit" class="btn btn-info" value="查询"
						onclick="changePage('frist')"> &nbsp&nbsp<a
						href="<c:url value="/OrderController?operate=list"/>"
					...
					<tbody>
						<c:forEach items="${orderInfoList}" var="orderInfo">
							<tr>
								...
								<td>${orderInfo.user.loginName}</td>
								...
								<td align="center"><c:if test="${orderInfo.isPay==0}">
										<a href="<c:url value="/OrderController?operate=updateOrder&isPay=1&orderId=${orderInfo.orderId}"/>"
											class="btn btn-2">确认付款</a>
									</c:if> <c:if test="${orderInfo.isDeliver==0&&orderInfo.isPay==1}">
										<a href="<c:url value="/OrderController?operate=updateOrder&isDeliver=1&orderId=${orderInfo.orderId}"/>" class="btn btn-2">发货</a>
									</c:if> <a data-toggle="modal" class="btn btn-2"
