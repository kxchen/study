...
<div class="list_three">
    ...
    <a href="<c:url value="UserAction?operate=listOrder&state=yudingzhong"></c:url>" class="sel">预约中</a>
    <a href="<c:url value="UserAction?operate=listOrder&state=zulinzhong"></c:url>">租赁中</a>
    <a href="<c:url value="UserAction?operate=listOrder&state=yiwancheng"></c:url>">已完成</a>
</div>
...
<c:forEach items="${orderDetailList}" var="orderDetail">
	<li>
        ...
        <div class="right">
            <i> <span>${orderDetail.reaPrice}</span></i>元/天<br>
            <c:if test="${orderDetail.isAbolish==0 }">
            <a href="<c:url value="UserAction?operate=updateOrder&orderDetailId=${orderDetail.orderDetailId}"></c:url>" class="a">取消</a>
            </c:if>
